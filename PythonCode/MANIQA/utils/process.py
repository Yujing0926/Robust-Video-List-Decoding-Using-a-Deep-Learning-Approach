...
def split_dataset_our_data(txt_file_name, split_seed=20):
    np.random.seed(split_seed)
    object_data = []
    with open(txt_file_name, 'r') as listFile:
        for line in listFile:
...
class ToTensor_f2(object):
    ...
    def __call__(self, sample):
        ...
        d_img = torch.from_numpy(d_img).type(torch.FloatTensor)
        score = torch.from_numpy(score).type(torch.FloatTensor)
        d_img_intact = sample['d_img_in']
        intact_score = sample['intact_score']
        d_img_intact = torch.from_numpy(d_img_intact).type(torch.FloatTensor)
        intact_score = torch.from_numpy(intact_score).type(torch.FloatTensor)
        sample = {
            'd_img_org': d_img,
            'score': score,
